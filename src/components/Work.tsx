        ...
        <div className="relative w-full">
          ...
          >
            {projects.map((project) => (
              <motion.div
                key={project.title}
                className="group border rounded-[1.5rem] p-5 md:p-6 bg-bg-warm text-plum border-plum/10 shadow-xl flex flex-col justify-between w-full text-left"
                  ...
                  <div className={project.links.length === 2 ? "grid grid-cols-2 gap-3" : "w-full"}>
                    {project.links.map((link) => (
                      <a 
                        ...
                        className="py-2 border border-plum/10 rounded-lg hover:bg-plum hover:text-bg-warm transition-all duration-300 flex items-center justify-center space-x-1.5 text-xs uppercase tracking-wider font-sans font-bold text-plum bg-plum/[0.02]"
                      >
